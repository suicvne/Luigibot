    ...
    public class IRC
    {
        ...
        public void Connect(string IrcServer, int IrcPort)
        {
            ...
            while (true)
            {
                ...
                while ((ircCommand = this.IrcReader.ReadLine()) != null)
                {
                    ...
                    else if (commandParts.Length > 3 && commandParts[3].Contains(":!slap"))
                    {
                        this.IrcCheckSlap(commandParts);
                    }
        ...
        private void IrcNamesList(string[] IrcCommand)
        {
            ...
            for (int intI = 5; intI < IrcCommand.Length; intI++)
            {
                ...
                newUsersList.Add(IrcCommand[intI]);
            }
		...
		if(elapsedTime == 5000)
{
		elapsedTime = 0;
            ...
            {
                Console.WriteLine("Ignoring unknown command error");
                return;
            }

            if(IrcCommand.Length >= 5) //in theory someone should exist...right?
            {
                string sender = fullUserID.Split(new char[] { '!' }, 2)[0];
                string recipient = IrcCommand[4];

                bool exists = false;
                foreach(String user in this.UserList)
                {
                    String actual = user.Trim(new char[] { ':', '@'});
                ...
                {
                    IrcWriter.WriteLine(String.Format("PRIVMSG {0} :What'd I do? :(", IrcChannel));
                    IrcWriter.Flush();
                    return;
                }
                if (exists)
